...
- History: retained presenter history for the focused provider, including a daily token usage heatmap (GitHub-style week grid where fill intensity comes from local token totals and each cell's outline shows that day's peak-quota band, plus a today marker and a legend), quota bars, local token summaries, and per-model quota for model-meter providers; days with no quota sample or local usage are omitted and the view shows its empty state when no day remains
