...
- History: presenter-rendered retained daily history for the focused provider, including a token usage heatmap (week-aligned day grid with intensity from daily local token totals) and per-model quota for model-meter providers; days with no quota sample or local usage are omitted and the view falls back to its empty state.
